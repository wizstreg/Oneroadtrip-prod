      ...
      if (state.steps && state.steps.length > 0) {
        state.steps.forEach(step => {
          if (step.place_id && window.PLACES_INDEX[step.place_id]) {
            ...
            // SAUF en mode import (from=temp) : le JSON importe fait foi,
            // une etape volontairement vide (ex: transit Paris) doit rester vide.
            const isImportMode = new URLSearchParams(location.search).get('from') === 'temp'
              || !!(window.state && window.state._rawImportedItin);
            console.log('[FIX-PARIS] step=', step.place_id, '| import=', isImportMode, '| visits actuelles=', (step.visits||[]).length);
            if (!isImportMode) {
